...
var _a;
var colorette_default = createColors({
  useColor:
    ((_a = process.env) == null ? void 0 : _a.LISTR_DISABLE_COLOR) !== "1",
});
...
var _DefaultRenderer = class {
  constructor(tasks, options, renderHook$) {
    ...
    this.options = __spreadValues(
      __spreadValues({}, _DefaultRenderer.rendererOptions),
      this.options
    );
  }
  getTaskOptions(task) {
    return __spreadValues(
      __spreadValues({}, _DefaultRenderer.rendererTaskOptions),
  ...
  getSelfOrParentOption(task, key) {
    var _a2, _b, _c;
    return (_c =
      (_a2 = task == null ? void 0 : task.rendererOptions) == null
        ? void 0
        : _a2[key]) != null
      ? _c
  ...
  createRender(options) {
    ...
    if (
      options.tasks &&
      (renderTasks == null ? void 0 : renderTasks.trim().length) > 0
    ) {
      render.push(renderTasks);
    }
    if (
      options.bottomBar &&
      (renderBottomBar == null ? void 0 : renderBottomBar.trim().length) > 0
    ) {
      render.push((render.length > 0 ? EOL : "") + renderBottomBar);
    }
    if (
      options.prompt &&
      (renderPrompt == null ? void 0 : renderPrompt.trim().length) > 0
    ) {
      ...
      if (task.isEnabled()) {
        if (task.hasTitle()) {
          if (
            ...
            if (
              task.hasFailed() &&
              this.getSelfOrParentOption(task, "collapseErrors")
            ) {
              output = [
                ...output,
                this.formatString(
              ...
              task.isSkipped() &&
              this.getSelfOrParentOption(task, "collapseSkips")
            ) {
              output = [
                ...output,
                this.formatString(
                  this.addSuffixToMessage(
                    task.message.skip &&
                      this.getSelfOrParentOption(task, "showSkipMessage")
                      ? task.message.skip
                      : task.title,
                    "SKIPPED",
              ...
              task.isRetrying() &&
              this.getSelfOrParentOption(task, "suffixRetries")
            ) {
              output = [
                ...output,
                this.formatString(
                  this.addSuffixToMessage(
                    task.title,
                    `RETRYING-${task.message.retry.count}`
                  ),
                  this.getSymbol(task),
                  level
                ),
              ...
              task.isCompleted() &&
              task.hasTitle() &&
              (this.getSelfOrParentOption(task, "showTimer") ||
                this.hasTimer(task))
            ) {
              output = [
                ...output,
                this.formatString(
                  `${task == null ? void 0 : task.title} ${this.getTaskTime(
                    task
                  )}`,
                  this.getSymbol(task),
                  level
              ...
              output = [
                ...output,
                this.formatString(task.title, this.getSymbol(task), level),
              ];
            }
          } else {
            output = [
              ...
              this.formatString(
                task.title,
                colorette_default.red(figures.squareSmallFilled),
                level
              ),
            ];
          }
        ...
        if (task == null ? void 0 : task.output) {
          if (
            (task.isPending() || task.isRetrying() || task.isRollingBack()) &&
            task.isPrompt()
          ) {
            ...
            if (
              !((_b =
                (_a2 = this.bottomBar[task.id]) == null ? void 0 : _a2.data) ==
              null
                ? void 0
        ...
        if (
          ...
          (task.isPending() ||
            ...
            (task.isCompleted() &&
              ...
              !task.subtasks.some(
                (subtask) => subtask.rendererOptions.collapse === true
              )) ||
            task.subtasks.some(
              (subtask) => subtask.rendererOptions.collapse === false
            ) ||
            task.subtasks.some((subtask) => subtask.hasFailed()) ||
            task.subtasks.some((subtask) => subtask.hasRolledBack()))
        ) {
          const subtaskLevel = !task.hasTitle() ? level : level + 1;
          const subtaskRender = this.multiLineRenderer(
            task.subtasks,
            subtaskLevel
          );
          if (
  ...
  renderBottomBar() {
    if (Object.keys(this.bottomBar).length > 0) {
      this.bottomBar = Object.keys(this.bottomBar).reduce((o, key) => {
        ...
        this.bottomBar[key].data = this.bottomBar[key].data.slice(
          -this.bottomBar[key].items
        );
  ...
  getSymbol(task, data = false) {
    ...
    if (task.isPending() && !data) {
      ...
      return task.hasSubtasks()
        ? colorette_default.red(figures.pointer)
        : colorette_default.red(figures.cross);
    } else if (
      task.isSkipped() &&
      !data &&
      this.getSelfOrParentOption(task, "collapseSkips") === false
    ...
    return !data
      ? colorette_default.dim(figures.squareSmallFilled)
      : figures.pointerSmall;
  }
  addSuffixToMessage(message, suffix, condition) {
    return (condition != null ? condition : true)
      ? message + colorette_default.dim(` [${suffix}]`)
      : message;
  }
};
...
var _SimpleRenderer = class {
    ...
    this.eventTypeRendererMap = {
      ...
      ["MESSAGE" /* MESSAGE */]: (task, event) => {
        if (event.data.error) {
          ...
          this.log(
            `${colorette_default.red(figures.cross)}${title}: ${
              event.data.error
            }`
          );
          ...
          const title = _SimpleRenderer.formatTitle(task);
          const skip =
            task.title !== event.data.skip ? `: ${event.data.skip}` : "";
          this.log(
            `${colorette_default.yellow(
              figures.arrowDown
            )}${title} [${colorette_default.yellow(`skipped${skip}`)}]`
          );
        } else if (event.data.rollback) {
          const title = _SimpleRenderer.formatTitle(task);
          this.log(
            `${colorette_default.red(figures.arrowLeft)}${title}: ${
              event.data.rollback
            }`
          );
          ...
          const title = _SimpleRenderer.formatTitle(task);
          this.log(
            `[${colorette_default.yellow(`${event.data.retry.count}`)}]${title}`
          );
        }
      },
    };
    this.options = __spreadValues(
      __spreadValues({}, _SimpleRenderer.rendererOptions),
      options
    );
  }
  static now() {
  ...
  log(output) {
    const logOut = (msg) => {
      process[this.options.output].write(
        msg.endsWith(EOL2) ? msg : `${msg}${EOL2}`
      );
    };
    ...
    if (tasks == null ? void 0 : tasks.length) {
      tasks.forEach((task) => {
        task.subscribe((event) => {
          var _a2, _b;
          (_b = (_a2 = this.eventTypeRendererMap)[event.type]) == null
            ? void 0
            : _b.call(_a2, task, event);
        }, this.log);
      });
    } else {
...
var _VerboseRenderer = class {
  constructor(tasks, options) {
    ...
    var _a2, _b, _c, _d;
    if (
      ((_a2 = this.options) == null ? void 0 : _a2.logger) &&
      ((_b = this.options) == null ? void 0 : _b.options)
    ) {
      ...
      this.logger = new Logger({
        useIcons: (_d = this.options) == null ? void 0 : _d.useIcons,
      });
    }
    this.options = __spreadValues(
      __spreadValues({}, _VerboseRenderer.rendererOptions),
      this.options
    );
  }
  render() {
            ...
            (event) => {
              ...
              if (task.isEnabled()) {
                const taskTitle = task.hasTitle()
                  ...
                  : "Task without title.";
                if (
                  event.type === "SUBTASK" /* SUBTASK */ &&
                  task.hasSubtasks()
                ) {
                  this.verboseRenderer(task.subtasks);
                } else if (event.type === "STATE" /* STATE */) {
                  if (
                    ((_a2 = this.options) == null
                      ? void 0
                      : _a2.logEmptyTitle) !== false ||
                    task.hasTitle()
                  ) {
                    if (task.isPending()) {
                  ...
                  if (
                    ((_d = this.options) == null
                      ? void 0
                      : _d.logTitleChange) !== false
                  ) {
...
var Task = class extends Subject {
  constructor(listr, tasks, options, rendererOptions) {
    ...
    this.skip =
      (_d = (_c = this.tasks) == null ? void 0 : _c.skip) != null ? _d : false;
    this.enabledFn =
      (_f = (_e = this.tasks) == null ? void 0 : _e.enabled) != null
        ? _f
        : true;
  ...
  async run(context, wrapper) {
    ...
    try {
      const retryCount =
        ((_a2 = this.tasks) == null ? void 0 : _a2.retry) &&
        ((_b = this.tasks) == null ? void 0 : _b.retry) > 0
          ? this.tasks.retry + 1
          : 1;
      ...
      if ((_c = this.tasks) == null ? void 0 : _c.rollback) {
        ...
        if (
          ((_d = this.listr.options) == null
            ? void 0
            : _d.exitAfterRollback) !== false
        ) {
        ...
        if (
          ...
          (await assertFunctionOrSelf(
            (_e = this.tasks) == null ? void 0 : _e.exitOnError,
            context
          )) !== false
        ) {
...
async function createPrompt(options, settings) {
      ...
      Object.assign(option, {
        stdout:
          this instanceof TaskWrapper
            ? (_a2 = settings == null ? void 0 : settings.stdout) != null
              ? _a2
              : this.stdout()
            : process.stdout,
  ...
  if (settings == null ? void 0 : settings.enquirer) {
    ...
    try {
      enquirer = new (
        await Promise.resolve().then(() => __toESM(__require("enquirer"), 1))
      ).default();
    } catch (e) {
...
var TaskWrapper = class {
  ...
  async prompt(options) {
    var _a2;
    return createPrompt.bind(this)(
      options,
      __spreadValues(
        {},
        (_a2 = this.options) == null ? void 0 : _a2.injectWrapper
      )
    );
  }
...
var Listr = class {
    ...
    tasks.forEach((task2) => {
      this.tasks.push(
        new Task(
          ...
          this.options,
          __spreadValues(
            __spreadValues({}, this.rendererClassOptions),
            task2.options
          )
        )
      );
  ...
  async run(context) {
    ...
    this.ctx =
      (_c =
        (_b = (_a2 = this.options) == null ? void 0 : _a2.ctx) != null
          ? _b
          : context) != null
        ? _c
        : {};
