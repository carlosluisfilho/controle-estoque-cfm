...
describe("GET /dashboard", () => {
  ...
  it("trata erro no servidor", async () => {
    // Mock para simular erro no service
    const dashboardService = require('../services/dashboardService');
    jest.spyOn(dashboardService, 'obterTotais').mockRejectedValue(new Error('Erro de teste'));
    
    const res = await request(app).get("/dashboard");
    expect(res.statusCode).toBe(500);
    expect(res.body.error).toBe('Erro de teste');
    
    dashboardService.obterTotais.mockRestore();
  });
});
